...
import { createExpense, createProvider, fetchExpenses, fetchLibros, fetchProviders, updateExpense } from '../../services/api';
...
export default function ExpensesPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const payload = { ...form, descripcion: `Compra: ${form.descripcion_producto.trim()}`, provider: Number(form.provider), libro: Number(form.libro), cantidad, valor_pagado: valorPagado, valor_unitario: valorPagado / cantidad };
      if (editingId) {
      ...
      setEditingId(null); setForm({ ...initial, fecha: form.fecha, provider: form.provider, libro: form.libro }); await load();
    }
    catch (error) { setMessage({ ok: false, text: error.response?.data?.error || 'No se pudo registrar el egreso.' }); }
  };
  const editExpense = (expense) => {
    setEditingId(expense.id);
    setForm({ provider: expense.provider, descripcion: expense.descripcion, fecha: expense.fecha, descripcion_producto: expense.descripcion_producto, valor_unitario: expense.valor_unitario, valor_pagado: expense.valor_pagado, cantidad: expense.cantidad, libro: expense.libro });
    setMessage(null);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  const cancelEdit = () => { setEditingId(null); setForm({ ...initial, provider: providers[0]?.id || '', libro: libros[0]?.id || '' }); };
