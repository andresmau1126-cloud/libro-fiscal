          ...
          <div className="data-table p-3">
            ...
            <form onSubmit={checkout}>
              ...
              <div className="row g-2">
                <div className="col-md-4"><label className="form-label small">Cliente registrado</label><select className="form-select" value={clienteId} onChange={(event) => { const selected = clientes.find((item) => String(item.id) === event.target.value); setClienteId(event.target.value); setCliente(selected?.nombre || ''); setClienteNit(selected?.nit || ''); }}><option value="">Nuevo / consumidor final</option>{clientes.map((item) => <option key={item.id} value={item.id}>{item.nombre} | NIT {item.nit}</option>)}</select></div>
                <div className="col-md-3"><label className="form-label small">Nombre del cliente</label><input className="form-control" value={cliente} onChange={(event) => { setClienteId(''); setCliente(event.target.value); }} placeholder="Nombre" /></div>
                <div className="col-md-3"><label className="form-label small">NIT</label><input className="form-control" value={clienteNit} onChange={(event) => { setClienteId(''); setClienteNit(event.target.value); }} placeholder="NIT" maxLength="50" /></div>
